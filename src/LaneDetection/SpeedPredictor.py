...
class SpeedPredictor:
    ...
    class DetectorParams:
        ...
        proc_img_size: Tuple[int, int] = (200, 200)
    ...
    def __init__(self, region: np.ndarray,
        ...
        self._params: SpeedPredictor.DetectorParams = params
    ...
    def _test_cnt(self, cnt, sh, tr):
        leftmost = tuple(cnt[cnt[:, :, 0].argmin()][0])
        rightmost = tuple(cnt[cnt[:, :, 0].argmax()][0])
        topmost = tuple(cnt[cnt[:, :, 1].argmin()][0])
        bottommost = tuple(cnt[cnt[:, :, 1].argmax()][0])
        if leftmost[0] <= tr or sh[1] - rightmost[0] <= tr or topmost[1] <= tr or sh[0] - bottommost[1] <= tr:
            return False
        else:
            return True
    def _get_xy_speed(self, warped) -> (float, float):
        # frame = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)
        th = self._img_filter(warped)
        cnts = cv2.findContours(th, cv2.RETR_EXTERNAL,
                                cv2.CHAIN_APPROX_SIMPLE)[-2]
        cnts = [i for i in cnts if cv2.contourArea(i) > 300]
        cnts = [i for i in cnts if self._test_cnt(i, th.shape, 10)]
        # print("cnts", len(cnts))
        v = cv2.drawContours(warped.copy(), cnts, -1, (255, 0, 0), 2)
        ...
        deltas = []
        if self._prev_objs_n is not None and len(self._prev_objs_n) > 0:
            for ob in objs_n:
                o = np.array(ob)
                dists = np.sum((np.array(self._prev_objs_n) - o)**2, axis=1)**0.5
                amin = np.argmin(dists)
                if dists[amin] < 60:
                    deltas += [np.array(o-self._prev_objs_n[amin])]
                # else:
                    # deltas += [None]
            if len(deltas) > 0:
                deltas = np.array(deltas)
                # speed = np.array(deltas)
                f1 = deltas[:, 0].mean()
                f2 = deltas[:, 1].mean()
                #     f_real = np.array([f2, f1])
                speed = np.array([f2, f1]) * [
                    (np.max(self.region[:, 0]) - np.min(self.region[:, 0])) / self._params.proc_img_size[1],
                    (np.max(self.region[:, 1]) - np.min(self.region[:, 1])) / self._params.proc_img_size[0]] / (
                                    time.time() - self._pr_t)

                self._slid_w.insert(0, speed)
                self._last_ts = time.time()
                # print("self._slid_w.", self._slid_w)

                # print("speedspeedspeedspeed", speed)

        # print(deltas)
        # for pp, d in zip(objs_n, deltas):
        #     if d is not None:
        #         cv2.circle(v, (int(
        #             pp[0]), int(pp[1])), 5, (int(np.sum(d))*20, 255, 0), 1)
        #     else:
        #         cv2.circle(v, (int(
        #             pp[0]), int(pp[1])), 5, (0, 0, 255), 1)

        self._prev_objs_n = objs_n


        # print()
        cv2.imshow("speedpred", v)
    ...
    def run(self, frame: np.ndarray) -> (float, float):
        ...
        self._slid_w = self._slid_w[:6]
        #
        # self._slid_w.append([vx, vy])
        # self._slid_w = self._slid_w[:50]
        if len(self._slid_w) > 0 and self._last_ts is not None:
            if time.time() - self._last_ts < 0.8:
